			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					if (data == "true" && document.getElementById("txtUploadFile<?php echo $mid ?>").value != "") {
						...
						if (data == '<?php echo JText::_('FAIL CREDENTIALS'); ?>') {
...
function startProcess<?php echo $mid ?>() {
	...
	if (!selPathSet<?php echo $mid ?> && document.getElementById("div_simplefileuploadpaths<?php echo $mid ?>").style.display=="block") {
		...
		alert("<?php echo JText::_('ADD PATH'); ?>");
		return false;
	}
...
function addFile<?php echo $mid ?>() {
	...
	if (rowcnt >= <?php echo $upload_maxmulti; ?>) {
		...
		alert("<?php echo JText::_('MAX MULTI REACHED'); ?>");
		return false;
	}
...
function chkUserPass<?php echo $mid ?>() {
	...
	if (user.length == 0 || pass.length == 0) {
		alert("<?php echo JText::_('UPLOAD USER'); ?>");
		return false;
	}
	jQuery("a.sfu_AChkCredentials<?php echo $mid ?>").trigger("click");
...
<div style="display: none;">
	<div id="sfuUploadProgress<?php echo $mid ?>" class="sfu_content" style="text-align: center; margin-bottom: 10px; margin-top: 10px; margin-left: 10px; margin-right: 10px; width: 220px;">
		<table border=0 style="width: 100%;">
			<tr>
				<td id="popProgress<?php echo $mid ?>" style="text-align: center;">
...
<div>
	...
	<table id="sfuContentTbl<?php echo $mid ?>" border="0" cellpadding="0" cellspacing="0">
		
		<?php if (($upload_username != "") && ($usr_id == 0) && (strcmp($_SESSION["upload_username_ok$mid"], md5($upload_password)) != 0)) { 
			$divUploadShow = "none";
		
		?>
		<tr class="logintr<?php echo $mid ?>"><td><?php echo JText::_('FILE LABEL'); ?></td></tr>
...
<!-- Added position:relative DIV due to IE bug -->
				<div style="position: relative;">
					<div id="div_simplefileuploaduser<?php echo $mid ?>" class="sfu_content" style="position: relative; width: 150px; padding: 10px 30px; margin: 10px auto; left: -28px; top:0px; display: block; background: <?php echo $upload_bgcolor ?>; text-align: left; border: 1px outset white; z-index: 20;">
						<table border=0 style="width: 100%;">
							<tr><td colspan="2" style="padding-bottom: 10px;"><b><?php echo JText::_('UPLOAD USER'); ?></b></td></tr>
							<tr><td><?php echo JText::_('USERNAME'); ?></td><td><input type="text" style="width: 71px;" size="10" id="txtUploadUser<?php echo $mid ?>" /></td></tr>
							<tr><td><?php echo JText::_('PASSWORD'); ?></td><td><input type="password" style="width: 70px;" size="10" id="txtUploadPass<?php echo $mid ?>" /></td></tr>
							<tr><td colspan="2" style="text-align: right;"><input type="button" value="OK" onclick="javascript:chkUserPass<?php echo $mid ?>();" /></td></tr>
						</table>
					</div>
				</div>
			</td>
		...
		<tr class="uploadtr<?php echo $mid ?>" style="display: <?php echo $divUploadShow; ?>">
			<td><?php echo JText::_('FILE LABEL'); ?> <?php if ($upload_multi == 1) { ?><span style="cursor: hand; cursor: pointer;" onclick="javascript:addFile<?php echo $mid ?>()"><b>[&nbsp;+&nbsp;]</b></span> <?php } ?></td>
		</tr>
		<tr class="uploadtr<?php echo $mid ?>" style="display: <?php echo $divUploadShow; ?>">
			<td>


				<table id="sfuContentTblInner<?php echo $mid ?>" border="0" cellpadding="0" cellspacing="0">
		...
		<tr id="trfileList<?php echo $mid ?>" style="display: none;">
			<td>
	...
	<?php if ($upload_capthca == 1) { ?>
		<tr class="uploadtr<?php echo $mid ?>" style="display: <?php echo $divUploadShow; ?>">
			<td>
				...
				<br/>
				<span style="font-size: 7pt;" ><?php echo JText::_('CASE INSENSITIVE'); ?></span>
				<?php } ?>
			</td>
		</tr>
	...
	?>
		<tr class="uploadtr<?php echo $mid ?>" style="display: <?php echo $divUploadShow; ?>">
			<td style="padding-top: 5px;">
				<input type="button" style="" onclick="javascript:startProcess<?php echo $mid ?>();" value="<?php echo JText::_('UPLOAD BUTTON TEXT'); ?>" />
				...
				<div id="div_simplefileuploadpaths<?php echo $mid ?>" class="sfu_content" style="padding: 10px 30px; margin: 10px auto; position: relative; left:-20px; top:-50px; display: none; background: <?php echo $upload_bgcolor ?>; text-align: left; border: 1px outset white; z-index: 20;">
					<table border=0 style="width: 100%;">
						<tr><td colspan="2"><nobr><u><?php echo JText::_('SELECT DIR'); ?>:</u></nobr></td></tr>
						<?php
						$ix = 0;
						foreach ($upload_userpath as $path) {
							
							echo "<tr><td><input type=\"radio\" name=\"selPath$mid\" value=\"".$ix."\" onclick=\"javascript:getCheckedValue".$mid."(this);\" /></td><td><nobr>".$path."</nobr></td></tr>";
							$ix += 1;
						}
						echo "<input type=\"hidden\" id=\"selPathId$mid\" name=\"selPathId$mid\" value=\"\" style=\"display: none;\" />";
						?>
						<tr><td colspan="2" style="text-align: right;"><input type="button" value="<?php echo JText::_('OK BUTTON'); ?>" onclick="javascript:processStep3<?php echo $mid ?>();" /></td></tr>
					</table>
				</div>
				<?php 
					} else {
				...
				?>
					...
					<div style="display: none;">
						<div id="sfuFormFields<?php echo $mid ?>" style="width:400px;overflow:auto;">
							<span style="text-align: left; color: #fff;" class="fancybox-bar-under"><?php echo JText::_('FORM FIELDS LABEL'); ?></span>
							<table id="sfuffTab<?php echo $mid ?>" border="0">
							<tr><td>
								<table border="0" width="100%">
									<tr><td colspan="2" style="background-color: #bdbdbd; border: 1px outset white;"><span style="font-style: italic;" id="sfuffFileName<?php echo $mid ?>_0"></span></td></tr>
								<?php
								$fields = explode(";", $upload_formfields);
								foreach ($fields as $f) {
									echo "<tr><td>".$f."</td><td><input type=\"text\" name=\"sfuff" . $mid . "_" . $f . "_rplcfilename\" style=\"width: 280px;\" /></td></tr>";
								}
								?>
								</table>
							</td></tr>
							</table>
							<span style="text-align: right;" class="fancybox-bar-under"><input type="button" onclick="javascript: sfufancyClose<?php echo $mid ?>();"  value="<?php echo JText::_('UPLOAD BUTTON TEXT'); ?>" /></span>
						</div>
					</div>
				<?php
				}
...
<?php
 if (($upload_popcaptchamsg == 0) && ($results === JText::_('FAULTY CAPTCHA'))) {
	...
	<div style="display: none;">
		<table id="sfuUploadMsg<?php echo $mid ?>" class="sfu_content" style="margin-right: 10px; margin-top: 10px; margin-left: 10px; width: 350px; height:252px;" cellspacing=0 cellpadding=0>
			<tr>
				<td valign="top" style="width: 100%; height: 25px; font-weight: bold; font-size: 12pt; color: #898998;">
					<?php echo JText::_('FILE UPLOAD NAME'); ?>
				</td>
				<td style="height: 25px; width: 100%;">&nbsp;</td>
			</tr>
			<tr>
				<td valign="top" style="width: 100%; font-size: 9pt; color: #898998;">
					<hr />
