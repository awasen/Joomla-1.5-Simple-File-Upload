...
$usr_id = $user->get('id');
// Check if username or name should be used for the UND directories
if ($params->get( 'upload_useundusername', '0' ) == 0)
...
if(is_array($settingidsudd) && ($usr_name !== "")) {
	...
	foreach($settingidsudd as $value){
		...
		if($value==="0") {
			//nothing
			if ($upload_debug == 1) echo "<br/>UDD value 0!";
		} else {
			
			//$name_chk = substr($value, 0, strpos($value, ">"));
			$name_chk = explode(">", $value);
			if ($upload_debug == 1) echo "<br/>Name check value=".$value." for UDD.";
...
if (strlen($filename) > 0) {
	...
	if ($upload_createdir == 1) { 
		if (!file_exists($upload_location)) {
			...
			if (mkdir($upload_location, 0777, true)) {
				...
				$results = JText::_('NEW DIR FAILED');
	...
	if ($tmp_upload_capthca == 0) {
		if ($_SESSION["uploaderr$mid"] == 0) {
			...
			$results .= ModSimpleFileUploaderHelperv13::getUploadForm($params, $upload_location, $sfu_basepath, $mid, $upload_users);
		...
		$_SESSION["uploaderr$mid"] = 1;
		$results = JText::_('FAULTY CAPTCHA');
	}
