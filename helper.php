					...
					if (($filetypeok) && ($_FILES["uploadedfile$mid"]["size"][$cnt] < $upload_maxsize)) {
						...
						if ($_FILES["uploadedfile$mid"]["error"][$cnt] > 0) {
							if (($_FILES["uploadedfile$mid"]["size"][$cnt] == 0) && ($_FILES["uploadedfile$mid"]["error"][$cnt] == 2)) {
								$errmsg = "(<span style='color:#dd2222'>".$_FILES["uploadedfile$mid"]["name"][$cnt].")</span>".JText::sprintf('ERROR TOO BIG', "<br />[PHP Error: " . $_FILES["uploadedfile$mid"]["error"][$cnt]) . "]<br />";
							} else {
								$errmsg = "(<span style='color:#dd2222'>".$_FILES["uploadedfile$mid"]["name"][$cnt].")</span>".JText::sprintf('ERROR LABEL', $_FILES["uploadedfile$mid"]["error"][$cnt]) . "<br />";
							}
							if ($upload_showerrmsg == 1) 
								$results .= $errmsg;
							else
								$results .= JText::_('UPLOAD FAILED');
							$fileErr = 1;
						} else {
							...
							$results .= "<span style='color:#55dd55'>".JText::sprintf('FILE UPLOAD LABEL', $_FILES["uploadedfile$mid"]["name"][$cnt]) . "</span><br />";
							$results .= JText::sprintf('FILE TYPE LABEL', $_FILES["uploadedfile$mid"]["type"][$cnt]) . "<br />";
							$results .= JText::sprintf('FILE SIZE LABEL', $filesize) . "<br />";
							#$results .= "uploaded to: " . $_FILES["uploadedfile$mid"]["tmp_name"][$cnt] . "<br />";
					  		if (file_exists($upload_location . $_FILES["uploadedfile$mid"]["name"][$cnt])) {
								if ( $upload_fileexist === "0" ) {
									// FAIL
									$results .= "<br /><strong>" . JText::sprintf('FILE EXISTS MSG', $_FILES["uploadedfile$mid"]["name"][$cnt]) . "</strong><br /><br />" . JText::_('FILE EXISTS CORR');
									$fileErr = 1;
								}
							...
							if ( $chkfileexist === "no" ) {
									...
									if (($img = @getimagesize($_FILES["uploadedfile$mid"]["tmp_name"][$cnt])) && (((is_numeric($upload_maximgwidth) && $upload_maximgwidth > 0) || (is_numeric($upload_maximgheight) && $upload_maximgheight > 0)) || ($upload_thumbcreate == 1))) {
										...
										switch ($type) {
											case 1: //'image/gif'
												if (imagetypes() & IMG_GIF)  { // not the same as IMAGETYPE
													...
													$errmsg = "GIF ".JText::_('IMG TYPE FAIL')."<br />";
												}
												break;
											case 2: //'image/jpeg'
												if (imagetypes() & IMG_JPG)  {
													...
													$errmsg = "JPEG ".JText::_('IMG TYPE FAIL')."<br />";
												}
												break;
											case 3: //'image/png'
												if (imagetypes() & IMG_PNG)  {
													...
													$errmsg = "PNG ".JText::_('IMG TYPE FAIL')."<br />";
												}
												break;
											case 15: //'image/wbmp'
												if (imagetypes() & IMG_WBMP)  {
													...
													$errmsg = "WBMP ".JText::_('IMG TYPE FAIL')."<br />";
												}
												break;
											default:
												$errmsg = $type." ".JText::_('IMG TYPE FAIL')."<br />";
												break;
										}
										...
										if ($errmsg === "") {
												...
												if (is_array($img_thumbsize)) {
													if (count($img_thumbsize) == 2) {
														if (is_numeric($img_thumbsize[0]) && is_numeric($img_thumbsize[1])) {
															...
															$results .= JText::_('IMG THUMB FILE') . ' <a href="'.$baseurl.str_replace(" ", "%20", $thumbfilename).'" target="blank">'.$thumbfilename.'</a><br/>';
											...
											if ($upload_maximgheight !== 0 && $upload_maximgwidth !== 0) {
												...
												if ($image_resize)
													$results .= JText::_('IMG ORIG RESIZE')."<br />";
												else
													$results .= JText::_('IMG RESIZE FAIL')."<br />";
											} else {
												$image_resize = false;
											}
											...
											$fileErr = 1;
											$results .= JText::_('FAIL REQUEST') . "<br />";
											$_SESSION["failedfile"] .= $_FILES["uploadedfile$mid"]["name"][$cnt]." (".JText::_('IMG SAVE FAIL').", ".$errmsg.")</br />";
										}
									}
									...
									if (($img = @getimagesize($_FILES["uploadedfile$mid"]["tmp_name"][$cnt])) && (!$image_resize) && ($upload_compressimg !== "") && is_numeric($upload_compressimg)) {
										...
										if (($type == 2) && (is_numeric($img_compressimg)) && ($fileErr == 0)) {
											...
											if (imagejpeg($oim, $upload_location . $_FILES["uploadedfile$mid"]["name"][$cnt], $img_compressimg)) {
												...
												$fileErr = 1;
												$results .= JText::_('FAIL REQUEST') . "<br />";
												$_SESSION["failedfile"] .= $_FILES["uploadedfile$mid"]["name"][$cnt]." (".JText::_('IMG COMPRESS FAIL').")</br />";
											}
										...
										if ($image_resize)
											$results .= JText::_('IMG ORIG COMPRESS')."<br />";
										else
											$results .= JText::_('IMG COMPRESS FAIL')."<br />";
									}	
								}
								...
								if (($upload_useformsfields == 1) && (strlen($upload_formfields) > 0) && ($fileErr == 0)) {
									...
									if ((strlen($valfile) > 0) && (strlen($upload_formfieldsfile) == 0)) {
										...
										if ($fileErr == 1) {
											$results .= JText::_('FAIL FORMFIELDS FILE') . "<br />";
										} else {
								...
								if (($upload_popshowpath == 1) && ($fileErr == 0)) {
									$results .= JText::sprintf('FILE SAVE AS', '<a href="'.$baseurl.str_replace(" ", "%20", $_FILES["uploadedfile$mid"]["name"][$cnt]).'" target="blank">'.$baseurl.$_FILES["uploadedfile$mid"]["name"][$cnt].'</a>').'<br /><br />';
								}
						...
						if (($upload_unzip == 1) && ($fileErr == 0)) {
							if (($_FILES["uploadedfile$mid"]["type"][$cnt] === "application/x-tar") || ($_FILES["uploadedfile$mid"]["type"][$cnt] === "application/x-tar-compressed") || ($_FILES["uploadedfile$mid"]["type"][$cnt] === "application/tar-compressed")) {
								...
								if ($res === FALSE) {
									$results .= "<p>".JText::_('MSG UNZIP ERROR')."</p>";
								} else {
									$results .= "<p>".JText::_('MSG UNZIP')."</p>";
								}
							}
						...
						$fileErr = 1;
						$errmsg = "(<span style='color:#dd2222'>".$_FILES["uploadedfile$mid"]["name"][$cnt].")</span><div>".JText::sprintf('FILE IN ERROR', $filetype)."<br />&nbsp;&nbsp;[".ModSimpleFileUploaderHelperv13::errCodeToMessage($_FILES["uploadedfile$mid"]["error"][$cnt])."]</div><br />";
						$_SESSION["uploaderr$mid"] = 1;
						if ($upload_showerrmsg == 1)
							$results .= $errmsg;
						else
							$results .= JText::_('UPLOAD FAILED')."<br /><br />";
						
						if ($written == 0) {
							$filesize = ModSimpleFileUploaderHelperv13::getFileSizePP($upload_maxsize);
							$results .= JText::_('ALLOWED TYPES') . ": " . $upload_filetypes . "<br />" . JText::_('FILE MAX SIZE') . ": " . $filesize . "<br /><br />";
							//$results .= "<div style=\"width: 90%; text-align: right;\"><input type='button' value='" . JText::_('OK BUTTON') . "' onclick='document.getElementById(\"div_simplefileuploadmsg\").style.display=\"none\";'></div>";
							$written = 1;
						}
					}
...
//echo "valfile=$valfile<br/>";
			...
			if (($upload_showdircontent == 1) && ($fileErr == 0)) {
				...
				if($bib = @opendir($upload_location)) {
					...
					if(is_array($fil_list)) {
						...
						$liste = "<li>" . JText::_('NO FILES FOUND') . " " . $upload_location . "</li>";
					}
					$results .=  "<h2>" . JText::_('FILES IN DIR') . " (" . $upload_location . "):</h2><ul>" . $liste . "</ul>";
				} else {
					//die("Could not read files in " . $upload_location);
					$results .=  "<h2>" . JText::_('NO FILES FOUND') . "</h2><br/>";
				}
				$results .= "</div><br/>";
			}
			...
			if ((strlen($upload_email) > 0) && ($fileErr == 0)) {
				...
				if ($upload_emailhtml == 0) {
					...
					$body = JText::sprintf('MAIL BODY', $_SERVER["HTTP_HOST"]);
					$body .= "\r\n\r\n".$text;
					...
					$body .= " ".JText::sprintf('BY USER', $users_name);
					$body .= $text;
				...
				if (mail($to, $subject, $body, $headers)) {
					...
					if ($upload_emailmsg == 1)
						$results .= "<p>".JText::_('MSG FAILED')."(To:".$to.")</p>";
				}
			}
	...
	function getContent($action) {
		...
		if ($action === "sfuuser") {
			...
			if ($session_username === "") {
				...
				$retVal = JText::_('FAIL CREDENTIALS');
			} else {
				...
				if ((strcmp($user, $session_username) == 0) && (strcmp($pass, $hashedpw) == 0)) {
					...
					$retVal = JText::_('USER PASS FAILED');
					/* debug					$retVal .= "\nGiven user = " . $user;
					$retVal .= "\nGiven pass = " . $pass;
		...
		if ($action === "sfucaptcha") {
			...
			if (strlen($captchaStored) > 0) {
				if ($captchaStored === $captcha)
					...
					$retVal = JText::_('FAULTY CAPTCHA');
			} else {
				$retVal = JText::_('FAIL REQUEST');
			}
		}
